...
public class NextDnsRewriteExclusionParser {
    ...
    public Optional<NextDnsRewriteExclusionConfig> parse(String envValue) {
        ...
        try {
            ...
            if (cleanupExisting) {
                rejectBroadCleanupPatterns(patterns);
            }
    ...
    private void rejectBroadCleanupPatterns(List<String> patterns) {
        for (String pattern : patterns) {
            String normalizedPattern = pattern.toLowerCase().strip();
            if (normalizedPattern.startsWith("www.")) {
                normalizedPattern = normalizedPattern.substring("www.".length());
            }

            String patternWithoutDots = normalizedPattern.replace(".", "");
            if (!patternWithoutDots.isEmpty() && patternWithoutDots.chars().allMatch(symbol -> symbol == '*')) {
                throw invalidConfiguration(
                        "NEXTDNS_REWRITE_EXCLUSIONS.cleanupExisting=true rejects broad pattern: " + pattern
                );
            }
        }
    }
