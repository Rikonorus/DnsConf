...
[Настройка редиректов (опционально)](#настройка-исключений-редиректов-опционально)
...
Добавьте домены в **переменную среды** `EXCLUDE_REDIRECT` через запятую **без пробела**, например: `instagram.com,twitch.com`
...
### Ограничения
Все профили получат _одинаковые_ настройки. Другими словами, значения в `BLOCK`, `REDIRECT` и `EXCLUDE_REDIRECT` будут **общими**.
...
В дополнение к настройке выше, укажите имя провайдера для каждого профиля через запятую **без пробела** в **переменной среды** `DNS`. Пример:
